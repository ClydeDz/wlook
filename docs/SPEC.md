...
This is the SPEC document. This README is the source of truth for product requirements, technical decisions, and the build plan.
...
| F4  | Multi-word selections (e.g. _"run away"_) are looked up as a phrase first; if no phrase entry exists, fall back to the first word.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| F12 | All user data (settings, downloaded dictionaries) lives under `%APPDATA%\Wlook\` so uninstall is clean.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               || F13 | The **global hotkey is fully customisable** from the dashboard Settings panel. The user can re-bind it (e.g. `CommandOrControl+Shift+D` → `Alt+L`), disable it entirely, or reset to default. If a chosen combination is already registered by another app or Windows itself, the UI shows a clear "conflict — pick another" error rather than failing silently.                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
- **Primary:** a configurable global hotkey (default `CommandOrControl+Shift+D`) that looks up the current selection. This works in every app.
...
2. **Preferred dialect** — radio between installed dialects (e.g. English UK / English US). A short explainer sits next to it: _"Lookups search every English dictionary you have installed. This setting decides which spelling and pronunciation Wlook shows first when both have a match."_
...
- Expect a **SmartScreen** warning ("Windows protected your PC") on first run on someone else's machine. Click _More info → Run anyway_. This is normal for unsigned builds.
...
- `core/dictionary/resolver`: cross-dialect merge — _color_ found in en-US when en-GB is preferred, _colour_ found in en-US when only en-US is installed (graceful), dedupe of identical senses across packs, preferred-dialect-first ordering, `sources` array populated correctly.
...
**Decision rule:** we build pure-Electron first. If `npm run measure` shows we're consistently above 150 MB idle on a clean Windows VM and we've exhausted V8/heap-tuning, _then_ we cut over to the Rust-agent variant. We do not pre-optimise.
